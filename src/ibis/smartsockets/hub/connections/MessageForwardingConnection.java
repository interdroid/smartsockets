...
public abstract class MessageForwardingConnection extends BaseConnection {
	...
	protected final static Logger meslogger = LoggerFactory
			.getLogger("ibis.smartsockets.hub.messages");

	protected final static Logger vclogger = LoggerFactory
			.getLogger("ibis.smartsockets.hub.connections.virtual");
	...
	protected void forward(ClientMessage m, boolean setHops) {
		...
		stats.infoMessages++;
		stats.infoMessagesBytes += m.messageSize();
		...
		if (hub == null) {
			...
			if (result.size() == 0) {
				...
				if (m.returnToSender) {
					if (meslogger.isDebugEnabled()) {
						meslogger
								.debug("Cannot return message to sender, since it "
										+ " has disappeared! (source= "
										+ m.targetAsString()
										+ " target="
										+ m.sourceAsString());
					}
					...
					if (meslogger.isDebugEnabled()) {
						meslogger
								.debug("Return message to sender, since target is "
										+ " not known! (source= "
										+ m.sourceAsString()
										+ " target="
										+ m.targetAsString());
	...
	protected final void handleCreateVirtual() throws IOException {
		...
		if (vclogger.isDebugEnabled()) {
			vclogger.debug("Creating virtual connection " + source + " -->> "
					+ target);
		}
	...
	protected final void handleMessageVirtual() throws IOException {
		...
		if (this == vc.mfc1) {
			...
			if (vclogger.isInfoEnabled()) {
				vclogger.info("forward message " + index + " " + vc.index2
						+ " (" + vc.index1 + ")");
			}
			...
			if (vclogger.isInfoEnabled()) {
				vclogger.info("forward message " + index + " " + vc.index1
						+ " (" + vc.index2 + ")");
			}
	...
	protected final void handleACKACKCreateVirtualConnection()
			throws IOException {
	...
	protected final void handleDataMessage() throws IOException {
		...
		try {
			...
			meslogger.warn("Failed to process data message: [" + data.length
					+ "]", e);
		}
	}
	...
	private VirtualConnection createConnection(
			MessageForwardingConnection mfc1, String id1, long index1,
			int fragment1) {
	...
	private void processVirtualConnectNACK(long index, byte reason) {
		...
		stats.connectionsReplies++;
		stats.connectionsNACKs++;
	...
	private void processVirtualConnectACKACK(long index, boolean succes) {
		...
		if (vc == null) {
			...
			// System.err.println("****** ACK ACK LOST " + index + " " +
			// succes);
	 ...
	 * connectionsFailed); System.out.println(name + " - lost : " +
	 * connectionsRepliesLost); System.out.println(name + " - error : " +
	 * connectionsRepliesError); System.out.println(name + " ");
	 ...
	 * System.out.println(name + " "); System.out.println(name + " VMessages : " +
	 * messages); System.out.println(name + " - bytes : " + messagesBytes);
	 * System.out.println(name + " - lost : " + messagesLost);
	 * System.out.println(name + " - error : " + messagesError);
	 * System.out.println(name + " "); System.out.println(name + " VMess. ACKS: " +
	 * messageACK); System.out.println(name + " - lost : " + messageACKLost);
	 * System.out.println(name + " - error: " + messageACK_Error);
	 * System.out.println(name + " "); System.out.println(name + " IMessages : " +
	 * infoMessages); System.out.println(name + " - bytes : " +
	 * infoMessagesBytes); System.out.println(name + " - delivered : " +
	 * infoMessagesDelivered); System.out.println(name + " - forwarded : " +
	 * infoMessagesForwarded); System.out.println(name + " - failed fw : " +
	 * infoMessagesFailed); System.out.println(name + " - returned : " +
	...
	protected final boolean runConnection() {
		...
		try {
			...
			case MessageForwarderProtocol.CREATE_VIRTUAL_ACK_ACK:
				if (meslogger.isInfoEnabled()) {
					meslogger
							.info("HubConnection got virtual connect ACK ACK!");
				}
